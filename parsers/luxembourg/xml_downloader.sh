...
while test "$url" ; do
    ...
    rm /tmp/$$.html
done | awk -F ';' 'BEGIN{print "cd docs/legilux"} {output=$1 ; gsub(/.*etat./, "", output); gsub(/\//, "_", output); gsub(/.xml/, "", output); pdf=$1 ; gsub(/xml$/, "pdf", pdf); print "wget -nc -q -O "output".xml \""$1"\" || wget -nc -q -O "output".pdf \""pdf"\""}' | sh

find docs/legilux -size 0 -exec rm '{}' ';'
